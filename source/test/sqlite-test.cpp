...
namespace Sql = RS::Sqlite;
using namespace RS::UnitTest;
using namespace std::literals;

void test_rs_sqlite_connection() {

    std::cout << "... Sqlite compile version: " << format_range(Sql::compile_version()) << "\n";
    std::cout << "... Sqlite runtime version: " << format_range(Sql::runtime_version()) << "\n";

    Sql::Connect dbc;
    Sql::Query query;
    Sql::Result result;

    TEST_THROW_MESSAGE(Sql::Connect("NoSuchFile"), Sql::SqliteError,
        "Sqlite error 14 in sqlite3_open_v2(): unable to open database file");

    TRY(dbc = Sql::Connect(Sql::Connect::memory));
    TRY(dbc.run(
        "create table tango("
    ...
    TEST_THROW_MESSAGE(query.run(), Sql::InvalidArgument,
        "Not all parameters set in query (0 out of 2 set)");
    TEST_THROW_MESSAGE(query.bind(42), Sql::InvalidArgument,
    ...
    TEST_EQUAL(count, 3);
    TEST_THROW_MESSAGE(dbc("select count(*) from Nonesuch"), Sql::SqliteError,
